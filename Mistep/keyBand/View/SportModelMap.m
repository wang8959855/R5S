...
@implementation SportModelMap

+(SportModelMap *)setValueWithDictionary:(NSDictionary *)dictionary
{
    
    SportModelMap *model = [[SportModelMap alloc]init];
    ...
    if (!((NSNull *)dictionary[TRAILARRAY] == [NSNull null]))
        model.trailArray = [NSKeyedUnarchiver unarchiveObjectWithData:dictionary[TRAILARRAY]];
    
    model.moveTarget = dictionary[MOVETARGET];
    model.mileageNumber = dictionary[MILEAGEM];
...
{
    return [NSString stringWithFormat:@"sportID = %@,sportType = %@,sportDate = %@,fromTime = %@,toTime = %@,heartRate = %@,heartRateArray = %@,stepNumber = %@,mileageNumber = %@,kcalNumber = %@,stepSpeed = %@,sportName = %@,deviceId = %@,deviceType = %@,isUp = %@,_haveTrail = %@,_trailArray = %@,_moveTarget = %@,_mileageM_map = %@,_sportPace = %@,_whenLong = %@",_sportID,_sportType,_sportDate,_fromTime,_toTime,_heartRate,_heartRateArray,_stepNumber,_mileageNumber,_kcalNumber,_stepSpeed,_sportName,_deviceId,_deviceType,_isUp, _haveTrail,_trailArray,_moveTarget,_mileageM_map,_sportPace,_whenLong];//,_mileageM = %@ _mileageM
}
