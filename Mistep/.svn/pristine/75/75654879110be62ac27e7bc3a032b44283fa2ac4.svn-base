...
@interface MapSportTypeSelectViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MapSportTypeSelectViewController
...
-(void)callbackSelect
{
    ...
    [self.navigationController popViewControllerAnimated:YES];
    if ([self.delegate respondsToSelector:@selector(callbackSelected:andSportModel:andSportName:)])
        [self.delegate callbackSelected:1000  andSportModel:self.sport andSportName:self.sport.sportName];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SportTypeTableViewCell *cell = [SportTypeTableViewCell cellWithSportModelTableView:tableView];
    cell.backgroundColor = kColor(246, 246, 246);
    [cell SportModelRefresh:indexPath.row+1];
    
    if (self.sportType == indexPath.row + 1)
    {
        cell.markName = @"xuanze";
    }
    if (indexPath.row > 5&& self.sportType > 6)
    {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SportTypeTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    cell.selected = NO;
    if(indexPath.row <=5 )
    {
        self.sportType = (int)indexPath.row + 1;
        self.sportName = cell.titleName;
        self.sport.sportName = cell.titleName;
        self.sport.sportType = intToString((int)indexPath.row + 1);
    }
    if (indexPath.row > 5)
    {
