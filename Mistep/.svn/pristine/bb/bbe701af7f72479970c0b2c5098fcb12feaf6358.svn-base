...
- (IBAction)saveChangeAction:(UIButton *)sender
{
    ...
    {
        [self showAlertView:NSLocalizedString(@"请选择开始时间", nil)];
        return;
    }
    else if (_endTimeBtn.titleLabel.text.length == 0)
    {
        [self showAlertView:NSLocalizedString(@"请选择结束时间", nil)];
        return;
    }else{
        ...
        {
            [self showAlertView:NSLocalizedString(@"结束时间不能早于或等于开始时间", nil)];
            return;
        }
        else if (endHour *60 + endMin - beginHour * 60 - beginMin < _duration)
        {
            [self showAlertView:NSLocalizedString(@"久坐时间段长度短于久坐提醒时间间隔", nil)];
            return;
        }
        else
        {
            if ([CositeaBlueTooth sharedInstance].isConnected)
            {
                ...
                [[CositeaBlueTooth sharedInstance] setSedentaryWithSedentaryModel:model];
                [self.navigationController popViewControllerAnimated:YES];
            }
        }
    }
